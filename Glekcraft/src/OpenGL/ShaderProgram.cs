...
public class ShaderProgram : IDisposable {
    ...
    public static ShaderProgram FromSources(GL context, string vertex, string fragment) {
        ...
        return program.AttachShader(Shader.FromSource(context, ShaderType.VertexShader, vertex))
    ...
    /// </param>
    /// <returns>
    /// Whether this instance has the given shader attached to it.
    /// </returns>
    ...
    public ShaderProgram DetachShader(Shader shader) {
        if (IsDisposed) {
            throw new ObjectDisposedException(nameof(ShaderProgram), "Cannot detach an OpenGL shader from a disposed OpenGL shader program");
        }
        if (!IsValid) {
            throw new InvalidOperationException("Cannot detach an OpenGL shader from an invalid OpenGL shader program");
        }
        if (!HasAttachedShader(shader)) {
            return this;
        }
        if (!shader.IsValid) {
            throw new InvalidOperationException("Cannot detach an invalid OpenGL shader from an OpenGL shader program");
        }
        Context.DetachShader(ID, shader.ID);
        var err = (ErrorCode)Context.GetError();
        if (err != ErrorCode.NoError) {
            throw new GLException(err, "Failed to detach an OpenGL shader from an OpenGL shader program");
        }
    ...
    /// <summary>
    /// Deactivate this instance from usage.
    /// </summary>
    /// <returns>
    /// This instance for chaining.
    /// </returns>
